...
router.get('/discord-id/:discordID', async (req, res, next) => {
  try {
...
router.get('/user-id/:userID/blacklist', async (req, res, next) => {
  try {
...
router.post('/user-id/:userID/blacklist', async (req, res, next) => {
  try {
    const blacklistUser = await User.blacklist(
      req.params.userID,
      req.body.userWhoBanned,
      req.body.reason, new Date(),
    );
    responseHandler(res, { blacklistUser });
  } catch (err) {
    errorHandler(res, err);
...
router.del('/user-id/:userID/blacklist', async (req, res, next) => {
  try {
...
router.put('/user-id/:userID/role/:roleID', async (req, res, next) => {
  try {
