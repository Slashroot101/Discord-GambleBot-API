...
router.post('/', async function(req, res, next) {
	try {
		...
		if(command) {
			const updatedCommand = await Commands.updateDurationAndUsage(command.id, req.body.duration, req.body.usages);
...
router.get('/:commandID/user/:userID/cooldown', async function(req, res, next) {
	try {
		const audit = await Commands.getCommandHistoryCountByDuration(req.params.commandID, req.params.userID);
		let onCooldown;
		let oldestAudit;
		if(audit !== undefined
